...
El repositorio se ha configurado para que se *ejecuten los tests automáticamente en la nube de GitHub cuando se realice un push o una pull request*. Para ello se hará uso del servicio [[https://github.com/features/actions][GitHub Actions]]. El fichero ~.github/workflows/nodeTests.yml~ contiene la configuración necesaria para activar la acción de ejecución de tests. Está convenientemente comentado. Se puede ver el resultado de la ejecución de los tests en la pestaña ~Actions~ del repositorio o bien en su página principal (un aspa roja indica fallo en algún test y un icono verde indica que se han superado). Fuente: https://docs.github.com/en/actions/guides/building-and-testing-nodejs
...
[[/home/pericon/codigo/practica_dwec_gestor_presupuesto/enunciados/imagenes/activar_actions.png]]
...
10. Ejecutar el comando ~npm test~ para lanzar los tests. Este comando podrá ejecutarse tantas veces como sea necesario. Por pantalla se mostrarán los tests que se pasan y los que no, de manera que se tendrá información sobre las acciones que hay que realizar. Los tests también se ejecutarán automáticamente en GitHub Actions al subir los cambios al repositorio y al realizar la pull request.
